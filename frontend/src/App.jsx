// frontend/src/App.jsx - Versión Final Funcional (Revisada para Sintaxis)

import React, { useState, useEffect, useCallback } from 'react';
import { IDKitWidget } from '@worldcoin/idkit';
import { verifyWorldId, confirmPaymentOnBackend, fetchMyTickets, fetchLatestResults } from './services/api';
import CountdownTimer from './components/CountdownTimer'; // Asegúrate que el Timer está importado
import './App.css';
...
function App() {
  ...
  const loadResults = useCallback(async () => { setIsLoading(true); setError(''); try { const res = await fetchLatestResults(); if (res?.data?.success && res?.data?.results) { setLastDraw(res.data.results); setError(''); } else { console.log("No valid results found:", res?.data?.message); setLastDraw(null); } } catch (err) { console.error("Error fetching results:", err); setError("Could not load results."); setLastDraw(null); } finally { setIsLoading(false); } }, []);

  // --- Manejadores ---
  const handleProof = useCallback(async (proofResponse) => { setIsLoading(true); setPaymentStatus('idle'); setError(''); try { const res = await verifyWorldId(proofResponse); if (res.data.success && res.data.nullifierHash) { setNullifierHash(res.data.nullifierHash); setIsVerified(true); loadMyTickets(res.data.nullifierHash); } else { const backendError = res?.data?.message || 'Verification failed on backend'; throw new Error(backendError); } } catch (err) { console.error("Verification Error:", err); let displayError = `Verification Error: ${err.message||'?'}`; if (err.message?.includes('already verified')) { displayError = "Already verified for this action."; } setError(displayError); setIsVerified(false); setNullifierHash(null); } finally { setIsLoading(false); } }, [loadMyTickets]);
      ...
      <div className="app-container card-texture">
          ...
          {!isVerified ? (
            <div className="action-box worldid-box">
              ...
              <IDKitWidget
                  ...
                  signal={worldcoinActionId} // Importante pasar el signal
                  handleVerify={handleProof}
                  onError={handleError}
                  credential_types={['orb']} // Intentar solo con orb
              >
                {({ open }) => <button onClick={open} disabled={isLoading || !worldcoinAppId || !worldcoinActionId} className="button button-primary button-worldid">Verify with World ID</button>}
              </IDKitWidget>
            ...
            <div className="action-box purchase-box">
              ...
              {paymentStatus !== 'success' ? (
                  <button
                      ...
                      className="button button-purchase" // Cambiado para diferenciarlo potencialmente
                  >
        ...
        {isVerified && (
          <section className="tickets-section data-section">
             {/* ... (Mostrar Mis Boletos - JSX sin cambios) ... */}
             <h2>My Tickets (Current Week)</h2> {Array.isArray(myTickets) && myTickets.length > 0 ? ( <ul className="item-list ticket-list">{ myTickets.map(ticket => (<li key={ticket?.id || Math.random()} className="list-item ticket-item"><span className="item-icon">🎫</span><span className="item-data">{Array.isArray(ticket?.numbers) ? ticket.numbers.join(' - ') : 'N/A'}</span></li>))}</ul> ) : (<p className="no-items-message">No tickets yet.</p>)}
          </section>
        )}

        <section className="results-section data-section">
            {/* ... (Mostrar Resultados - JSX sin cambios) ... */}
            <h2>Latest Draw Results ({lastDraw?.drawWeek?.replace('-W',' / W ')||'N/A'})</h2> {lastDraw && Array.isArray(lastDraw.winningNumbers) ? ( <div className="draw-details"> <div className="results-summary"><p><strong>Winning #:</strong> <span className="winning-numbers">{lastDraw.winningNumbers.join(' - ')}</span></p><p><strong>Pool:</strong> {lastDraw.potWld || 0} WLD</p></div> <h3>Winners:</h3> {lastDraw.results ? ( <ul className="item-list winners-list">{Array.isArray(lastDraw.results.first) && lastDraw.results.first.length > 0 ? (lastDraw.results.first.map((w, i) => ( <li key={`1-${w?.ticketId || i}`} className="list-item winner winner-first"><span className="item-icon">🥇</span> ...</li> )) ) : (<li className="list-item no-winner">(No 1st)</li>)}{Array.isArray(lastDraw.results.second) && lastDraw.results.second.length > 0 ? (lastDraw.results.second.map((w, i) => ( <li key={`2-${w?.ticketId || i}`} className="list-item winner winner-second"><span className="item-icon">🥈</span> ...</li> )) ) : (<li className="list-item no-winner">(No 2nd)</li>)}{Array.isArray(lastDraw.results.third) && lastDraw.results.third.length > 0 ? (lastDraw.results.third.map((w, i) => ( <li key={`3-${w?.ticketId || i}`} className="list-item winner winner-third"><span className="item-icon">🥉</span> ...</li> )) ) : (<li className="list-item no-winner">(No 3rd)</li>)}</ul> ) : ( <p className="no-items-message">Winner info unavailable.</p> )} </div> ) : (<p className="no-items-message">{isLoading && !error ? 'Loading...' : 'Results not available.'}</p> )}
        </section>
